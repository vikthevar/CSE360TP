...
public class ControllerUserLogin {
	...
	*/
	...
	protected static void doLogin(Stage ts) {
		...
		// Validate the username using the FSM recognizer before using it
		String userNameError = UserNameRecognizer.checkForValidUserName(username);
		
		// Update existing UI to provide detailed feedback on invalid input
		if (!userNameError.isEmpty()) {
			// Provide a message for invalid input based on FSM and halt
			ViewUserLogin.alertUsernamePasswordError.setContentText(userNameError.trim());
